...
class LessonCRUDTestCase(APITestCase):
    def setUp(self):
        self.owner = User.objects.create_user(email='owner@example.com', password='pass12345', is_active=True)
        self.stranger = User.objects.create_user(email='stranger@example.com', password='pass12345', is_active=True)
        self.moderator = User.objects.create_user(email='moder@example.com', password='pass12345', is_active=True)
        moderators_group, _ = Group.objects.get_or_create(name='moderators')
        self.moderator.groups.add(moderators_group)

        self.course = Course.objects.create(title='Django', description='...', owner=self.owner)
        self.lesson = Lesson.objects.create(
            course=self.course, title='Models', description='...',
            video_url='https://www.youtube.com/watch?v=abc', owner=self.owner,
        )
...
class SubscriptionTestCase(APITestCase):
    def setUp(self):
        self.user = User.objects.create_user(email='sub_user@example.com', password='pass12345', is_active=True)
        self.owner = User.objects.create_user(email='course_owner@example.com', password='pass12345', is_active=True)
        self.course = Course.objects.create(title='DRF', description='...', owner=self.owner)

    def test_anonymous_cannot_subscribe(self):
        response = self.client.post(reverse('lms:subscription'), {'course_id': self.course.id})
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
    ...
    def test_course_serializer_reflects_subscription(self):
        ...
        response = self.client.get(reverse('lms:course-detail', kwargs={'pk': self.course.id}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
