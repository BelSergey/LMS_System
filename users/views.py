...
def confirm_email(request, uidb64, token):
    ...
    if user is not None and email_confirmation_token.check_token(user, token):
        ...
        messages.success(
            request, "Email подтверждён. Теперь вы можете войти в систему."
        )
    else:
...
class EmailLoginView(LoginView):
    ...
    def form_invalid(self, form):
        messages.error(
            self.request, "Неверный email/пароль, либо email ещё не подтверждён."
        )
...
class PaymentStatusAPIView(APIView):
    ...
    def get(self, request, pk, *args, **kwargs):
        ...
        if not payment.session_id:
            return Response(
                {"detail": "Для этого платежа нет сессии Stripe."}, status=400
            )
